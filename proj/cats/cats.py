...
def about(topic):
    ...
    def contains(paragraph):
        ...
        # notice that the returned item is new object!
        # remove punctuation
        no_punc_para = remove_punctuation(paragraph)
        # lower the case
        lower_no_punc_para = lower(no_punc_para)
        # split the string
        para_words = split(lower_no_punc_para)
...
def accuracy(typed, reference):
    ...
    loop_len = min(type_len, ref_len)
    count = 0
    for i in range(loop_len):
        cur_type_word = typed_words[i]
        cur_ref_word = reference_words[i]
        if is_match(cur_type_word, cur_ref_word):
            count = count + 1
    return 100 * count / type_len
    # END PROBLEM 3
